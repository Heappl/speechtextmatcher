...
The keynote of this part of the thesis is a study of similarities between languages and their phonetics. I present here not only my own algorithms, but also certain parts of sphinx library, which I used: including tool for alignment problem using preprocessed audio model and dictionary, and in other case state scorers from trained English and Russian models. \newline
...
Humans can hear frequencies, that begins as low as 12Hz (under laboratory conditions) to 20kHz (for adults usually much lower). However speech range is a little bit smaller than that [3], as shown in the figure 1. \newline

\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.5]{speech_range.jpg}
    \caption[]{Speech range from \emph{Fundamentals of speech recognition} [3]}
\end{figure}
...
The conclusion from the anatomy of a human ear is that frequencies of the sound are important.
...
\newpage
In the figure 3 we can see sample functions and the result of Fourier Transform applied to them.
\begin{figure}[hb]
    \centering
...
The function's plot is given in the figure 4.
\begin{figure}[hb]
    \centering
...
\newpage
Note that it emphasises values in the middle of the window, so our actual windows should overlap to cover whole time domain. For example by shifting a window by a percentage of it actual width as in the figure 5.
\begin{figure}[hb]
    \centering
...
The figure 6 shows a plot of human speech signal in a frequency domain, which shows powers of signal at each frequency, and a plot in a time domain. [3]
\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.5]{speech_spectrogram.jpg}
    \caption[]{Example speech signal from \emph{Fundamentals of speech recognition} [3]}
\end{figure}
...
For example a cepstrum of an echoed signal in the figure 7 shows clearly a three 'quefrencies' of the echo of the signal. [12]
\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.4]{echo_cepstrum.jpg}
    \caption[]{Cepstrum of signal with echo from \emph{The Quefrency Alanysis of Time Series for Echoes} [12]}
\end{figure}
...
The main feature of Sphinx library is an ability of speech recognition using input audio model and dictionary. However it is a modular and open source library,
...
The given filters should look like in the figure 9.
\begin{figure}[hb]
    \centering
    \includegraphics[scale=0.4]{mel_filters.jpg}
    \caption[]{Melody filters from \emph{Fundamentals of speech recognition} [3]}
\end{figure}
...
differences, calculated as shown in the figure 10.
\begin{figure}[hb]
    \centering
...
Actual phones, that are classified as a single phoneme, make a diversified family.  Different variants of a phoneme are called allophones. For example /l/ in English "leap" and "deal" or polish examples of allophones (/ł/ in "umysł" might be voiceless contrary to "ławka") or vowels between soft consonants (/a/ in "jajko") [22], or an example, where two allophones can be easily recognized, in the word "koński": 'ń' before 's' has  to allophones: 'ń' and '\~{\j}', which is a nasal 'j', nevertheless since there are no two contexts, where both 'ń' and '\~{\j}' create different words, they are treated as a single phoneme. \newline
...
Some of English phonemes and example phones are shown in the figure 11.
\begin{figure}[hb]
    \centering
    \includegraphics[height=8.7cm, width=13cm]{example_phones_1.jpg}
    \includegraphics[height=8.7cm, width=13cm]{example_phones_2.jpg}
    \caption[]{Example phones from \emph{Fundamentals of speech recognition} [3]}
\end{figure}
...
\begin{itemize}
    \item Let's assume, that we have a training set of audio frames, that are assigned to different phonemes (and silence maybe).
    \item We would like to classify frames from a completely different audio stream as phonemes.
    \item By calculating a distance from each frame to each phoneme set, we can find the nearest set. The set's phoneme will be the one, we assign to the input frame.
\end{itemize}
...
A simpler approach would be to calculate correlations between populations (frequencies magnitudes or signal features) and use them in a distance measure.
...
However I also tried to see how it behaved with marked one occurence of the word, and then it was actually able to find other occurences, ... in one case out of five. \newline
...
The problem with this vector arises, when $\Sigma$ matrix is singular, which is not invertible nor it has determinant. This can happen quite often, when the observation data is too small. As we can see from the Appendix A.2, the covariance matrix is at least positive-semidefinitive, however in order to prevent degenerate cases we can allow only positive-definite matrices, and any such a matrix is guaranteed to be invertible. If we keep our training data large, then this should be common, that the covariance matrix will be invertible. \newline
...
I chose to focus solely on audiobook recordings throughout this project and in this problem it is no different.\newline
...
\begin{itemize}
    \item absolute value of the raw and normalized\footnote{average value of whole stream is 0} signal frame
...
One should choose wisely how to deal with it, since in theory it is possible to figure out even small pauses in the speech signal, like i.e. between syllables or maybe even pause, that are part of phoneme, like in stop (plosive) consonants ('t', 'd', 'k', 'g', 'b', 'p'). \newline
...
\end{equation}
Whetever the signal was marked as a part of speech signal, the background average is always updated (see Sphinx implementation for details). \newline
...
\begin{equation}
    B = \{ F \in S \: | \text{ there is no such } \sigma_i \in F \text{ where } \sigma_i > Ave_i\}
\end{equation}
...
Couple of details are to be dealt with. Not only alphanumeric characters make a word, i.e. a `'` is also a character, that must be treated as a part of speech character at this point, although in later phases it might be ignored. \newline
...
\begin {equation}
    E(\text{time to say }w) = 0.95 \: |w| \mu_{char} + 0.05 \: \mu_{word}
\end {equation}
...
The efficiency of above method was tested versus a word alignment obtained with method from section 5, where I calculate an alignment with word granularity using trained Russian audio model. I create statistic for two different variants:
\begin{itemize}
...
\begin{itemize}
    \item time differences:
    \begin{itemize}
        \item there were \textbf{154} chunks (30.6\%) which time frame were within a \textbf{0.5s} difference, 
    ...
    \item word differences:
    \begin{itemize}
        \item \textbf{179} (35.6\%) chunks had \textbf{0} difference in words
        \item \textbf{54} (10.7\%) chunks where different by \textbf{1} word (missing or additional)
        \item \textbf{20} (4\%) different by \textbf{2} words
        \item \textbf{56} (11.1\%) with a difference over \textbf{10} words
    \end{itemize}
    \begin{center}
...
\end{tabular}
\end{center}
\footnotetext{alternatively one can use 'o u'}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item if exists such $p \in P$, that it is currently matched, set $p$ to $c$
        \item if $P$ is empty
        \begin{itemize}
...
\begin{itemize}
    ...
    \item Corpora uses actual phonetics replacing voiced versions of i.e. “z”, “zh” with voiceless alternatives (“s”, “sz”) and vice-versa (gaining voice), \newline
...
On one end we have not so easy problem of combining spoken words and phrases into sentences and the `only' thing we need to do, is to choose words, that most naturally combine together, and then just match a silence part, so there is no sound artefact from merging words. \newline
...
I experimented with two algorithms for word synthesizing. First is a bit better for testing purposes of phoneme alignment, while the second produces better results. Both algorithms use audio recording tagged with phonemes as an input. \newline
...
The scoring function of merging two candidates: $c_{i,w_k} \in C_{w_k}$ for $k$-th word and $c_{j,w_{k+1}} \in C_{w_{k+1}}$ for $k+1$-th word, is a sum of euclidean distances between a small\footnote{i.e. a constant factor of size of candidates} number of sequential frames from the ending of $c_{i,w_k}$ and the beginning of $c_{j,w_{k+1}}$. \newline
...
The algorithm once again uses a dynamic programming technique in quite similar manner to the common part, and although the main difference is in the scoring function, the details about processed variables are not simple to present, without explaining whole algorithm.
...
The figure 23 shows intervals (between red lines) from two sequences, for which a distance is calculated, and if a closest frames were chosen as the arrow indicates, how a resulting stream would look like.
...
The more systemized tests of the speech synthesizer are proposed [31], but they all require human assessment. Example tests may look as follows:
\begin{itemize}
    \item writing down recognized synthesized sentences by human participants and counting committed errors
    \item subjective evaluation of naturality of synthesized speech by large number of listeners
\end{itemize}
...
\bibitem{} Benjamin Picart, Thomas Drugman, Thierry Dutoit \emph{Assessing the Intelligibility and Quality of HMM-based Speech Synthesis with a Variable Degree of Articulation}
\end{thebibliography}

\newpage
\begin{appendices}
\section{Phoneme modelling}
\subsection{Calculating parameters for single variable normal distribution}
